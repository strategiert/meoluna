...
**Base:** `https://mission-control-etj.pages.dev`
**Auth:** API-Key als **Query-Parameter** `?apiKey=6d85b4e7ddeb00134ea3b871b05ce8bf` (KEIN Authorization-Header, kein Bearer-Token!)
**Projekt:** `meoluna` — MUSS als Query-Parameter mitgehen (`&project=meoluna`), nicht nur im Body.
**Eigener Agent-Name als `owner`/`agent`:** z. B. `Codex`.

#### WICHTIG: Die 4 klassischen Fehler
1. `Authorization: Bearer …` → 401. Richtig: `?apiKey=…` in der URL.
2. `PATCH /api/tasks/<id>` (ID im Pfad) → **405**. Richtig: `PATCH /api/tasks?apiKey=…&project=meoluna&id=<ID>` (ID als Query-Param).
3. POST mit schon existierendem Titel → **409**. Erst per GET prüfen, ob es den Task gibt, dann PATCH statt POST.
4. **Windows + Umlaute:** `-d '{"notes":"…ä…"}'` inline zerstört UTF-8. Immer JSON in Datei schreiben (UTF-8) und mit `--data-binary @datei.json` senden.

#### Alle Tasks abrufen
```bash
curl -s "https://mission-control-etj.pages.dev/api/tasks?apiKey=6d85b4e7ddeb00134ea3b871b05ce8bf&project=meoluna"
```

#### Neuen Task anlegen (nur wenn Titel noch nicht existiert!)
```bash
cat > /tmp/task.json << 'EOF'
{"title":"<Aufgabe>","status":"In Progress","owner":"Codex","notes":"konkret: was, wo (Pfad/Commit), Stand, offene Punkte"}
EOF
curl -s -X POST "https://mission-control-etj.pages.dev/api/tasks?apiKey=6d85b4e7ddeb00134ea3b871b05ce8bf&project=meoluna"   -H "Content-Type: application/json" --data-binary @/tmp/task.json
# Antwort: { "ok": true, "id": "<TASK-ID>" }  -> ID merken für spätere PATCHes
```

#### Task updaten (Status/Notes ändern) — ID als Query-Param!
```bash
cat > /tmp/patch.json << 'EOF'
{"status":"Done","notes":"Ergebnis: was erreicht, Commit <sha>, Live-Status, offene Punkte"}
EOF
curl -s -X PATCH "https://mission-control-etj.pages.dev/api/tasks?apiKey=6d85b4e7ddeb00134ea3b871b05ce8bf&project=meoluna&id=<TASK-ID>"   -H "Content-Type: application/json" --data-binary @/tmp/patch.json
```

#### Chat-Message posten (Team-Sichtbarkeit)
```bash
cat > /tmp/msg.json << 'EOF'
{"agent":"Codex","content":"1-2 Sätze: was erledigt, Pfad/Commit, ggf. Handoff"}
EOF
curl -s -X POST "https://mission-control-etj.pages.dev/api/messages?apiKey=6d85b4e7ddeb00134ea3b871b05ce8bf"   -H "Content-Type: application/json" --data-binary @/tmp/msg.json
```

#### Status-Spalten
`Recurring` · `Backlog` · `In Progress` · `In Review` · `Blocked` · `Done`

#### Task-Felder
| Feld | Typ | Beschreibung |
|------|-----|--------------|
| `title` | string | Kurzer Titel (Pflicht; doppelter Titel = 409) |
| `status` | string | siehe Status-Spalten |
| `notes` | string | konkret: Pfad, Commit-Hash, Live-Status, offene Punkte — nie nur "erledigt" |
| `tags` | string | komma-separiert |
| `owner` | string | z. B. `Codex` |
| `due` | string | ISO-Datum (optional) |

#### Workflow für Agents
1. **Vor Arbeitsbeginn:** GET prüfen, ob passender Task existiert → sonst POST mit `status: "In Progress"`.
2. **Nach Abschluss:** PATCH auf `"Done"` + Ergebnis (Commit, Live-URL) in `notes` + Chat-Message posten.
