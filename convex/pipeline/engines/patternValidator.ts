import type { PatternEngineSpec, PatternRoom } from "./patternTypes";
...
export function validatePatternEngineSpec(spec: PatternEngineSpec): PatternValidationResult {
  ...
  for (const room of spec.rooms) {
    ...
    if (room.mode !== "continue" && room.mode !== "fill") violations.push(`E_ROOM_${label}: mode must be continue or fill`);
    ...
    room.rounds.forEach((r, i) => {
      ...
      // Eindeutigkeit: das Muster muss periodisch sein.
      ...
      } else if (period < 2 && new Set(r.sequence).size > 1) {
        // p==1 nur erlaubt, wenn alle gleich sind (trivial). Sonst kein Muster.
        violations.push(`E_ROOM_${rl}: pattern period too trivial`);
      }
      if (typeof r.gapIndex !== "number" || r.gapIndex < 0 || r.gapIndex >= r.sequence.length) {
        violations.push(`E_ROOM_${rl}: gapIndex out of range`);
        return;
      }
      const last = r.sequence.length - 1;
      if (room.mode === "continue" && r.gapIndex !== last) {
        violations.push(`E_ROOM_${rl}: continue mode requires gapIndex at the end (got ${r.gapIndex}, last ${last})`);
      }
      if (room.mode === "fill" && (r.gapIndex === 0 || r.gapIndex === last)) {
        violations.push(`E_ROOM_${rl}: fill mode requires gapIndex in the middle (got ${r.gapIndex})`);
      }
      const answer = r.sequence[r.gapIndex];
