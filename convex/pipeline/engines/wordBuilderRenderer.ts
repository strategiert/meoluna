...
// "Bilderbuch-Tag": helle, freundliche Spielwelt fuer Kinder ab 5.
const KID = {
  skyTop: '#79c7f5',
  skyBottom: '#e9f8ff',
  hillBack: '#a8dd8a',
...
function StarRow({ stars }) {
  return (
    <div className="flex items-center gap-1 rounded-full border-2 px-3 py-1 text-xl" style={{ background: KID.card, borderColor: KID.ink, color: KID.ink }}>
      <span>⭐</span><span className="kid-font font-extrabold">{stars}</span>
    </div>
  );
}
...
function RoundDots({ total, current }) {
  return (
    <div className="flex items-center gap-1.5 rounded-full border-2 px-3 py-1.5" style={{ background: KID.card, borderColor: KID.ink }}>
      {Array.from({ length: total }).map((e, i) => (
        <div key={i} className="h-3.5 w-3.5 rounded-full border-2" style={{ background: i < current ? KID.green : i === current ? KID.sun : '#e3e8f0', borderColor: KID.ink }} />
      ))}
    </div>
  );
}
...
function WordRoom({ room, roundIndex, onRoundWin, setBubble, setMood }) {
  ...
  const [pool, setPool] = useState(() => buildPool(round));
  ...
  function placeChip(chip) {
    ...
    if (chip.value === round.chips[nextSlot]) {
      ...
      if (nextSlot + 1 >= round.chips.length) {
        setSolved(true);
        speak(round.word);
        Meoluna.reportScore(10, { action: 'word-round-correct', roundIndex });
        onRoundWin();
      } else {
...
function RoomScene({ room, roomMeta, stars, onBack, onComplete, onStar }) {
  ...
  function handleRoundWin() {
    setMood('cheer');
    Meoluna.reportScore(10, { action: 'word-round-bonus', roomId: room.roomId, roundIndex });
    onStar();
    if (roundIndex + 1 >= room.rounds.length) {
      setPhase('done');
      setBubble(room.feedback.correct + ' ' + room.explanationAfterSuccess);
      Meoluna.reportScore(25, { action: 'word-room-complete', roomId: room.roomId });
    ...
    <div className="kid-font min-h-screen p-3 sm:p-6" style={{ background: 'linear-gradient(180deg, ' + KID.skyBottom + ', #f8fdf2)' }}>
      ...
      <div className="mx-auto flex max-w-5xl flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          <button type="button" onClick={onBack} className="rounded-2xl border-2 px-4 py-2 text-lg font-extrabold transition-all active:translate-y-0.5" style={{ background: KID.card, borderColor: KID.ink, color: KID.ink, boxShadow: '0 4px 0 ' + KID.slotEdge }}>← Karte</button>
        ...
        {phase !== 'done' && (
          <WordRoom key={roundIndex} room={room} roundIndex={roundIndex} onRoundWin={handleRoundWin} setBubble={setBubble} setMood={setMood} />
        )}
      ...
      <div className="relative mx-auto max-w-4xl">
        ...
        <div className="relative mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {SPEC.rooms.map((room, index) => {
            ...
            return (
              <button key={room.roomId} type="button" disabled={locked} onClick={() => onStart(index)} className="rounded-[1.8rem] border-4 p-5 text-center transition-all active:translate-y-1 disabled:opacity-50" style={{ background: done ? '#e8f9e4' : KID.card, borderColor: KID.ink, boxShadow: '0 6px 0 ' + KID.slotEdge }}>
                <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full text-3xl" style={{ background: done ? KID.green : locked ? '#dde3ec' : KID.sun }}>
                  {done ? '⭐' : locked ? '🔒' : isLast ? '🏆' : room.mode === 'syllables' ? '🧩' : '🔤'}
                </div>
...
export default function App() {
  ...
  if (activeRoomIndex !== null) {
    ...
    return (
      <RoomScene key={room.roomId} room={room} roomMeta={roomMeta} stars={stars} onBack={() => setActiveRoomIndex(null)} onComplete={completeActiveRoom} onStar={() => setStars((v) => v + 1)} />
    );
  }
