...
export function buildWorldCode(worldData: WorldData): string {
  ...
  return [
    ...
    `}`,
    `function getModuleBackground(moduleIndex) {`,
    `  const g = getGraphics();`,
    `  const bgs = Array.isArray(g.backgrounds) ? g.backgrounds : [];`,
    `  // bgs[0] = hub_bg, bgs[1+] = module_bg_0, module_bg_1, ...`,
    `  const moduleBg = bgs[moduleIndex + 1];`,
    `  if (moduleBg?.url) return moduleBg.url;`,
    `  // Fallback: hub bg or first available bg`,
    `  return (bgs[0]?.url) || WORLD_DATA.config?.hubBgUrl || null;`,
    `}`,
    ...
    `}`,
    ``,
    `// ── Voice stoppen (vor Navigation) ──`,
    `function stopVoice() { window.parent.postMessage({ type: 'meoluna:stop' }, '*'); }`,
    ``,
    ...
    ``,
    `  if (done) return (`,
    `    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-cover bg-center relative" style={getModuleBackground(moduleIndex) ? { backgroundImage: 'url(' + getModuleBackground(moduleIndex) + ')' } : undefined}>`,
    `      {cfg.hubBgUrl && <div className="absolute inset-0 bg-black/50 pointer-events-none" />}`,
    ...
    `  const moduleBgUrl = getModuleBackground(moduleIndex);`,
    ``,
    `  return (`,
    `    <div className="min-h-screen flex flex-col p-4 bg-cover bg-center" style={moduleBgUrl ? { backgroundImage: 'url(' + moduleBgUrl + ')' } : undefined}>`,
    ...
    ``,
    `  return (`,
    `    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-cover bg-center relative" style={cfg.hubBgUrl ? { backgroundImage: 'url(' + cfg.hubBgUrl + ')' } : undefined}>`,
    `      {cfg.hubBgUrl && <div className="absolute inset-0 bg-black/50 pointer-events-none" />}`,
