...
5. **Zwillings-Regel (Klaus-Entscheidung 2026-07-05):** Import-Topics, die ein Legacy-Topic inhaltlich duplizieren, stehen in `src/data/topic-canonicals.json` (`"import-key": "legacy-key"`). Sie bekommen ein Cross-Canonical auf die Hauptseite und KEIN noindex (Kombination wäre für Google widersprüchlich; Canonical konsolidiert die Signale), bleiben aber aus der Sitemap. Kompetenzbereichs-Überschriften (z. B. „Lesen k1–k4") sind KEINE Zwillinge — zu breit, bleiben noindex bis sinnvoll befüllt. Zwillinge bekommen keinen eigenen Authored-Content.
