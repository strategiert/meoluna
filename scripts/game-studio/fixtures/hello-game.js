...
export function bootMeolunaGame(context) {
  ...
  class MainScene extends window.Phaser.Scene {
    create() {
      ...
      const startBtn = this.add.rectangle(W / 2, H / 2, 300, 140, 0x4a7c59).setInteractive();
      const label = this.add.text(W / 2, H / 2, "Start", { fontSize: "48px", color: "#ffffff" }).setOrigin(0.5);
      context.api.setAffordances([{ id: "hello.start", x: W / 2 - 150, y: H / 2 - 70, width: 300, height: 140 }], { width: W, height: H });
      startBtn.on("pointerdown", () => {
        ...
        for (let i = 0; i < 3; i += 1) {
          const x = 170 + Math.floor(rnd() * (W - 340));
          const y = 170 + Math.floor(rnd() * (H - 340));
          const star = this.add.star(x, y, 5, 28, 65, 0xffd166).setInteractive();
          stars.push({ id: `hello.star-${i + 1}`, x: x - 65, y: y - 65, width: 130, height: 130, obj: star });
          star.on("pointerdown", () => {
            star.destroy();
            found += 1;
