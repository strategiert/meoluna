// Direkter Import via Fetch API
const CONVEX_URL = "https://merry-leopard-276.convex.cloud";
...
// Erst alle Subjects holen
async function main() {
  console.log("Fetching subjects...");
  const subjects = await convexQuery("curriculum:getSubjects");
  const subjectMap = {};
  ...
  for (const t of TOPICS) {
    const subjectId = subjectMap[t.s];
    if (!subjectId) {
      console.log(`Subject not found: ${t.s}`);
      continue;
    }

    try {
      await convexMutation("curriculum:createTopic", {
        subjectId,
        name: t.n,
        slug: slugify(t.n),
        gradeLevel: t.g,
      ...
      console.log(`\nError creating ${t.n}: ${e.message}`);
    }
  }

  console.log(`\nDone! Created ${created} topics.`);
}
